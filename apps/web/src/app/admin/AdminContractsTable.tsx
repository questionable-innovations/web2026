      ...
      <table className="w-full text-sm text-left">
        ...
        <tbody className="divide-y">
          {contracts.map((c, i) => {
            ...
            const aBalanceRaw = aTokenBalances?.[i]?.result as bigint | undefined;
            ...
            const principalFormatted = formatUnits(rawPrincipal, 6); // Assuming USDC (6 decimals)
            ...
            return (
              <tr key={c.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
                ...
                <td className="px-4 py-3 text-green-600 dark:text-green-400 font-medium">
                  {interestRaw > 0n ? `+$${Number(interestFormatted).toFixed(4)}` : "$0.0000"}
                </td>
                <td className="px-4 py-3">
